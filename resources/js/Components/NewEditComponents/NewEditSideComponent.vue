<template>
    <div>
        <div>
            <form @submit.prevent="onNewSideSubmit">
                <div class="grid grid-cols-1 gap-x-3">
                    <div class="flex flex-grow flex-col">
                        <InputCustom :translate="true" v-model="form.sideValue" required :label="__('strings.name')" id="name"
                                     name="name"
                                     :error="form.errors.sideValue" type="text"></InputCustom>
                    </div>
                    <div class="flex flex-row w-full justify-between">
                        <div class="flex justify-start gap-2 mt-3">
                            <n-button type="error"
                                      attr-type="button"
                                      @click="goToBack"
                                      :disabled="form.processing"> {{ __('strings.cancel') }}
                            </n-button>
                        </div>
                        <div class="flex justify-end gap-2 mt-3">
                            <n-button
                                attr-type="submit"
                                :disabled="form.processing">{{ __('strings.save') }}
                            </n-button>
                        </div>
                    </div>
                </div>
            </form>
        </div>
    </div>
</template>

<script setup>

import Multiselect from "vue-multiselect";
import InputCustom from "@/Components/Input.vue";
import {FontAwesomeIcon} from "@fortawesome/vue-fontawesome";
import {useForm, usePage} from "@inertiajs/inertia-vue3";
import {computed, getCurrentInstance, h, nextTick, onMounted, reactive, ref} from "vue";
import {NIcon, NTag, useDialog} from "naive-ui";
import {Inertia} from "@inertiajs/inertia";

onMounted(() => {
    if(props.sides){
        form.sideValue = props.sides.value
    }
})
const form = useForm({
    sideValue: null,
})



const props = defineProps({
    sides : Array
})

const dialog = useDialog();

const onNewSideSubmit = () => {
    if(props.sides){
        let langs = usePage().props.value.langs;
        let nullLang = false
        langs.forEach((lang)=>{
            if(nullLang == false) {
                if (form.sideValue[lang.value] == null || form.sideValue[lang.value] == '') {
                    nullLang = true
                    dialog.warning({
                        title: $trans('strings.attention'),
                        content: $trans('strings.nullLangs'),
                        positiveText: $trans('strings.confirm'),
                        negativeText: $trans('strings.cancel'),
                        maskClosable: false,
                        onPositiveClick: () => {
                            form.transform((el) => {
                                return ({
                                        sideValue: el.sideValue,
                                    }
                                )
                            }).put(route('warehouse.sides.update',{
                                id : props.sides.id
                            }),{
                                onFinish : () => {
                                    if (!form.hasErrors) {
                                        //Inertia.get(route('warehouse.versions.index'))
                                    }
                                }
                            })
                        }
                    })
                }
            }
        })
        if(!nullLang){
            form.transform((el) => {
                return ({
                        sideValue: el.sideValue,
                    }
                )
            }).put(route('warehouse.sides.update',{
                id : props.sides.id
            }),{
                onFinish : () => {
                    if (!form.hasErrors) {
                        //Inertia.get(route('warehouse.versions.index'))
                    }
                }
            })
        }
    }else{
        let langs = usePage().props.value.langs;
        let nullLang = false
        langs.forEach((lang)=> {
            if (nullLang == false) {
                if (!(form.sideValue instanceof Object) || form.sideValue[lang.value] == null || form.sideValue[lang.value] == '') {
                    nullLang = true
                    dialog.warning({
                        title: $trans('strings.attention'),
                        content: $trans('strings.nullLangs'),
                        positiveText: $trans('strings.confirm'),
                        negativeText: $trans('strings.cancel'),
                        maskClosable: false,
                        onPositiveClick: () => {
                            form.transform((el) => {
                                console.log(el)
                                return ({
                                        sideValue: el.sideValue,
                                    }
                                )
                            }).post(route('warehouse.sides.store'), {
                                onFinish: () => {
                                    if (!form.hasErrors) {
                                        //Inertia.get(route('warehouse.versions.index'))
                                    }
                                }
                            })
                        }
                    })
                }
            }
        })
        if(!nullLang){
            form.transform((el) => {
                console.log(el)
                return ({
                        sideValue: el.sideValue,
                    }
                )
            }).post(route('warehouse.sides.store'), {
                onFinish: () => {
                    if (!form.hasErrors) {
                        //Inertia.get(route('warehouse.versions.index'))
                    }
                }
            })
        }
    }
}

const goToBack = function() {
    window.history.back()
}

const str_replace = (str) => {
    if(str){
        str = str.toString();
        return str.replace('.',',')
    }

}
const formatDate = (temp) => {
    let date = new Date(temp)
    let y = date.getFullYear();
    let m = (date.getMonth() + 1) < 10 ? '0'+ (date.getMonth()+1) : date.getMonth()+1
    let d = date.getDate() < 10 ? '0' + date.getDate() : date.getDate()
    return y + '-' + m + '-' + d
}
</script>


<style scoped>

</style>
